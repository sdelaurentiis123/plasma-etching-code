...
**C11 (in progress):** the charging table (Q, V_f, E_defl vs AR) is being regenerated from petch's
own derived-source solver (`surface_charging="petch"`), replacing the published-closure table —
making notching first-principles end-to-end. **Limit:** absolute notch depth uncalibrated (3–5×
shallow of HG); sub-micron grid stability is a separate open item.
