...
def run(args):
    ...
    payload = {
        ...
        "runtime": {
            "warmup_wall_time_s": warmup_wall,
...
def parse_args(argv=None):
    ...
    total_physical = int(args.profile_steps) * float(args.step_duration_s)
    if (not np.isfinite(args.dx_nm) or args.dx_nm <= 0.0
        ...
        parser.error(
            "profiler requires 1--4 positive steps, <=0.1 s total physical time, "
            "and positive bounded controls")
    return args
