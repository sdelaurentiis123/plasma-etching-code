...
# Word-boundary matched like CONTENT_TOKENS below: "resonance"/"resonant" are
# ordinary physics (chlorine resonance radiation files) and must not trip the
# path gate; a bare "resona" path segment still refuses the build.
PATH_TOKENS='partner|(^|[^a-z])resona([^a-z]|$)|challenge'
LEAKED="$(tar tzf "$OUT" | grep -icE "$PATH_TOKENS" || true)"
if [ "$LEAKED" != "0" ]; then
  echo "REFUSING: $LEAKED sensitive path(s) present in $OUT" >&2
  tar tzf "$OUT" | grep -iE "$PATH_TOKENS" >&2
  rm -f "$OUT"
  exit 1
fi
