...
def _load_correction_derivation(path, endpoint_pair):
    ...
    if (payload.get("schema") != "petch.krueger-2024.base-coupled-correction.v1"
        ...
        raise ValueError(
            "10 nm endpoint is not bound to its coupled-correction derivation")
    return path, payload
...
def _load_r110_launch_manifest(path, endpoint_pair):
    ...
    if (payload.get("schema") != "petch.krueger-2024.r110-base-authority.v1"
            or payload.get("protocol_id") != "K24-PETCH-R1.10"
