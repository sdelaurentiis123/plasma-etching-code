...
def run(label, reinit, warm, nfp, gsmooth, gsrc, gmesh, samp, gws=False, dflux=False):
    p = dict(petch.PAR); p['n_fp'] = nfp
    p['flux_smooth_gpu'] = gsmooth; p['gpu_source'] = gsrc; p['gpu_mesh'] = gmesh
    p['gpu_warmstart'] = gws; p['device_flux'] = dflux
...
run("warmup", "fsm", True, 1, True, True, True, "sobol", gws=True, dflux=True)

base = run("ORIGINAL: cold n_fp=4, skfmm-CPU, CPU-smooth, sobol-host, CPU-mesh, KDTree",
           "skfmm", False, 4, False, False, False, "sobol", gws=False, dflux=False)
best = run("FULL GPU: warm n_fp=1, fsm, GPU-smooth/source/mesh/warmstart/device-flux",
           "fsm", True, 1, True, True, True, "sobol", gws=True, dflux=True)
