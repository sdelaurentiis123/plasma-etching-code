"""Atom-balanced Gray Ar+/F beam closure for silicon dioxide.

Gray's 1993 MIT thesis, Chapter 5, gives the same two-site construction for
SiO2 as for Si, but with independently measured coefficients:

``theta_F = s0 R / (s0 R + 2 beta2 (1 + b))``

``Y = p0 (1 - theta_F) + beta2 (1 + b) theta_F``

where ``R`` is the atomic-F/Ar+ particle-flux ratio and

``p0 = 0.0139 max(sqrt(E) - sqrt(18 eV), 0)``

``beta2 = 0.053 max(sqrt(E) - sqrt(4 eV), 0)``

``b = 0.007 sqrt(E/eV)``.

The source's reduced near-surface state is SiF2-like.  Ion-assisted removal
is split into the measured mass-spectrometric branches: an unsaturated SiF2
fragment plus O2, or SiF4 plus O2.  Consequently those branches consume two
and four F atoms per removed SiO2 formula unit, respectively.  Bare physical
sputtering consumes none.  This is the product-resolved atom ledger missing
from the legacy mixed-layer approximation.

The closure is beam-regressed and phenomenological, not an elementary
first-principles potential.  Gray explicitly calls the shared SiF2 surface
state a gross simplification and the two-new-site SiO2 cascade count
arbitrary.  The atom ledger below makes that published reduced topology
conservative; it does not upgrade the topology's evidence.  The closure is
valid only for normally incident Ar+ and atomic F on Gray's measured
energy/flux-ratio board and must not be silently transferred to reactive
fluorocarbon ions or off-normal feature impacts.
"""
...
class GrayArFSiO2State:
    ...
    def __post_init__(self):
        supplied = np.broadcast_arrays(
            ...
            np.asarray(self.removed_sio2_formula_units_m2, dtype=float),
    ...
    def with_conservative_surface_fields(self, fields):
        ...
        if set(fields) != set(self.conservative_surface_fields()):
            raise ValueError(
                "Gray Ar+/F silicon-dioxide remap fields do not match")
        return type(self)(
...
class GrayArFSiO2Parameters:
    ...
    def __post_init__(self):
        ...
        if (not self.ion_species or not self.neutral_species
            ...
            raise ValueError("invalid Gray Ar+/F silicon-dioxide parameters")
    ...
    def beam_energy_law(cls):
        thesis = (
            "gray-1993-thesis, Chapter 5: Tables 5-1 and 5-10; "
            "Eqs. 5-28, 5-30, 5-31, and 5-35"
        )
        return cls(
            ion_species="Ar+",
            neutral_species="F",
            initial_f_sticking_coefficient=0.02,
            physical_sputter_yield=SteinbruchelYield(0.0139, 18.0),
            fluorinated_surface_yield=SteinbruchelYield(0.053, 4.0),
            fragment_branch_per_sqrt_eV=0.007,
            bulk_sio2_formula_density_m3=2.2e28,
            minimum_measured_energy_eV=20.0,
            maximum_measured_energy_eV=2000.0,
            ...
            evidence={
                "initial_f_sticking_coefficient": ParameterEvidence(
                    ...
                    note=(
                        "Table 5-10 constant-s0 fit; apparent initial F "
                        "sticking on ion-renewed clean SiO2 sites"),
                ...
                "physical_sputter_yield": ParameterEvidence(
                    ...
                    note="Table 5-1 and Eq. 5-7: A=0.0139, Eth about 18 eV",
                    supports_prediction_within_declared_domain=True,
                ),
                "fluorinated_surface_yield": ParameterEvidence(
                    ...
                    "controlled-beam yield regression",
                    note=(
                        "Eq. 5-35: beta2=0.053(sqrt(E)-sqrt(4)); "
                        "Table 5-10 spans 20--2000 eV"),
                    supports_prediction_within_declared_domain=True,
                ),
                "fragment_branch": ParameterEvidence(
                    ...
                    note=(
                        "Eq. 5-28: summed unsaturated SiFx/SiF4 branch "
                        "represented as SiF2 in the source's reduced model"),
                    supports_prediction_within_declared_domain=True,
                ),
                "bulk_sio2_formula_density_m3": ParameterEvidence(
                    "2.20 g/cm3 amorphous SiO2 density and molar mass",
...
@dataclass(frozen=True)
class GrayArFSiO2StepResult:
    state: GrayArFSiO2State
    etch_velocity_m_s: np.ndarray
    fluorinated_fraction: np.ndarray
    physical_removed_formula_units_m2: np.ndarray
    sif2_removed_formula_units_m2: np.ndarray
    sif4_removed_formula_units_m2: np.ndarray
    ...
    def __post_init__(self):
        names = (
            ...
            "physical_removed_formula_units_m2",
            "sif2_removed_formula_units_m2",
            "sif4_removed_formula_units_m2",
            "consumed_f_atoms_m2",
            "incident_f_atoms_m2",
        ...
        for name in names:
            ...
            if np.any(~np.isfinite(value)) or np.any(value < 0.0):
                raise ValueError(
                    "invalid Gray Ar+/F silicon-dioxide step result")
            value.setflags(write=False)
            values[name] = value
        ...
        if (np.any(values["fluorinated_fraction"] > 1.0)
                ...
                or not isinstance(self.state, GrayArFSiO2State)
                or not isinstance(
                    self.material_exchange, SurfaceMaterialExchange)
                or not isinstance(self.validity, MechanismValidity)):
            raise ValueError(
                "invalid Gray Ar+/F silicon-dioxide result contract")
        ...
        removed = (
            values["physical_removed_formula_units_m2"]
            + values["sif2_removed_formula_units_m2"]
            + values["sif4_removed_formula_units_m2"]
        )
        expected = self.material_exchange.removed_units_m2["SiO2_formula"]
        if np.any(
            np.abs(expected - removed)
            > 64.0 * np.finfo(float).eps * np.maximum(removed, 1.0)
        ):
            raise ValueError(
                "Gray Ar+/F silicon-dioxide branches do not close")


class GrayArFSiO2Mechanism:
    """Normally incident, species-resolved Ar+/F/SiO2 beam closure."""

    quasi_steady_surface_state = True

    def __init__(self, parameters: GrayArFSiO2Parameters | None = None):
        self.parameters = (
            GrayArFSiO2Parameters.beam_energy_law()
            if parameters is None else parameters
        )
        if not isinstance(self.parameters, GrayArFSiO2Parameters):
            raise TypeError("parameters must be GrayArFSiO2Parameters")

    @staticmethod
    def initial_state(shape=()):
        return GrayArFSiO2State.bare(shape)

    @property
    def provenance(self):
        par = self.parameters
        return MappingProxyType({
            "model": "Gray-1993 normally-incident Ar+/F/SiO2 site model",
            "equations": [
                "5-7", "5-28", "5-30", "5-31", "5-35", "5-36",
            ],
            "parameters": {
                "initial_f_sticking_coefficient":
                ...
                "bulk_sio2_formula_density_m3":
                    par.bulk_sio2_formula_density_m3,
            },
            "declared_domain": {
            ...
            "source_evidence_ceiling": (
                "Gray calls the SiF2 surface representation a gross "
                "simplification and the two-new-site SiO2 cascade count "
                "arbitrary; conservative routing does not make the reduced "
                "topology atomistic"
            ),
        })

    def neutral_reaction_probability(self, state: GrayArFSiO2State):
        if not isinstance(state, GrayArFSiO2State):
            raise TypeError(
                "Gray neutral probabilities require GrayArFSiO2State")
        return MappingProxyType({
        ...
        if isinstance(population, FaceResolvedEnergeticFlux):
            reduced = np.bincount(
                ...
                weights=(
                    ...
                    * value_per_event(population.event_energy_eV)),
    ...
    def validity(self, state, fluxes):
        if not isinstance(state, GrayArFSiO2State):
            raise TypeError("Gray validity requires GrayArFSiO2State")
        par = self.parameters
        ...
        if unsupported_neutral or unsupported_energetic:
            reasons.append(
                "positive incident flux has no declared Ar+/F/SiO2 channel")
        if leaves_energy:
            reasons.append("ion energy leaves the measured 20--2000 eV board")
        if off_normal:
            reasons.append(
        ...
        if thermal_only:
            reasons.append(
                "F-only thermal SiO2 etching is not included in this beam card")
        ...
        return MechanismValidity(
            ...
            known_model_form_omissions=(
                ...
                "thermal F-only SiO2 etching is omitted from this ion-enhanced card",
                "the summed unsaturated SiFx signal is represented as SiF2",
                "co-emitted oxygen is represented as a product bundle",
                "no product launch energy-angle distribution was measured",
                "no off-normal ion response was measured",
                "the Ar+ card is not transferred to reactive fluorocarbon ions",
    ...
    def advance(
            self, state: GrayArFSiO2State, fluxes: SurfaceFluxes,
            duration_s: float, *, strict=True):
        if not isinstance(state, GrayArFSiO2State):
            raise TypeError("Gray mechanism requires GrayArFSiO2State")
        ...
        exchange = SurfaceMaterialExchange(
            ...
            outgoing_units_m2={"SiO2_formula": removed},
            unresolved_units_m2={},
            deposited_units_m2={},
            known_limitations=(
                "summed unsaturated SiFx branch is reduced to SiF2",
                "oxygen coproducts are bundled with their SiFx branch",
                "emission energy-angle distributions are not measured",
            ),
        )
        products = (
            SurfaceProductPopulation(
                "SiO2_physical", "SiO2_formula", physical_removed,
                1.0, 60.083,
                provenance={
                    "branch": "Gray Table 5-8 bare-SiO2 physical sputter",
                    "missing": "differential emission energy-angle law",
                },
            ),
            SurfaceProductPopulation(
                "SiF2_plus_O2_bundle", "SiO2_formula", sif2_removed,
                1.0, 98.080,
                provenance={
                    "branch": (
                        "Gray Table 5-8/Eq. 5-28 unsaturated SiFx branch "
                        "represented as the stoichiometric SiF2 + O2 bundle"),
                    "missing": (
                        "resolved SiFx distribution and differential "
                        "co-product emission"),
                },
            ),
            SurfaceProductPopulation(
                "SiF4_plus_O2_bundle", "SiO2_formula", sif4_removed,
                1.0, 136.076,
                provenance={
                    "branch": (
                        "Gray Table 5-8 ballistic-mixing SiF4 + O2 branch"),
                    "missing": "differential co-product emission",
                },
            ),
        )
        return GrayArFSiO2StepResult(
            state=updated,
            etch_velocity_m_s=(
                physical_rate + sif2_rate + sif4_rate
            ) / par.bulk_sio2_formula_density_m3,
            fluorinated_fraction=theta,
            physical_removed_formula_units_m2=physical_removed,
            sif2_removed_formula_units_m2=sif2_removed,
            sif4_removed_formula_units_m2=sif4_removed,
