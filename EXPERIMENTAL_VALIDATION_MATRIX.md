...
| Self-consistent backward charging | A partial; S failing when converged | **conservation bug fixed; reference mismatch exposed** | A corner cell was incorrectly updated once per exposed face, giving contradictory currents to one floating capacitor. Pooling all faces per insulating cell restores residual decay. Deterministic sub-face integration removes point-collocation as a frozen-field bias source. | With sub-face integration, AR4 iteration 30 still gives floor 51.3 V, foot energy 11.5 eV, residual RMS 0.100 and noisy max 0.396—not the forward references 34.0 V/21.6 eV. The earlier 5.0 V RMSE/corr 0.998 were under-converged transient outputs. Sampling and grid ladders, source normalization, and field resolution remain open. |
