...
| Charging/notching | Poisson with permittivity/conductors, deposited current, dielectric storage/conductivity, self-consistent current balance | Physical-time charge evolution and full charged surface-response/re-impact exist. A separate moving-surface remap conserves retained positive/negative charge and ledgers etched-away charge. Signed convergence-contract revision `CCA-2026-07-13-R2` authorizes their C3 co-evolution integration under refinement gates | Low-AR HG/notch lineages are separate and include reference-emulation closures | Absolute matched profile; grid/sample/init invariance; charging off/on | Treat HG as a causal/unit benchmark, not modern HARC validation; proceed through the signed C3 gates without reopening frozen-map root solvers |
