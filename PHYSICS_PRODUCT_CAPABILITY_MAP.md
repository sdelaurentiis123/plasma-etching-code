...
| Ion reflection | Material-, energy-, angle-, roughness-dependent scattering kernel | Missing | ViennaPS-like coned-cosine law and stochastic energy loss | Differential reflection distributions and profile ablation | Add a sourced interaction-table kernel with uncertainty; no universal cone law |
| Product redeposition | Species-resolved production, flight, reaction/sticking, and exact material balance | Surface mechanisms now return an exact material-origin ledger and mark unresolved product routing in validity; product populations and transport remain missing | Scalar velocity-derived products, fixed sticking, and a charging/etch-stop foot-band suppression hack | Time-resolved necking/taper and trench-to-hole transfer | Route only sourced products; unresolved removed material cannot redeposit; the foot-band branch must be retired |
...
| Charging/notching | Poisson with permittivity/conductors, deposited current, dielectric storage/conductivity, self-consistent current balance | Quasi-static 3-D dielectric loop exists and refuses nonconvergence | Low-AR HG/notch lineages are separate and include reference-emulation closures | Absolute matched profile; grid/sample/init invariance; charging off/on | Treat HG as a causal/unit benchmark, not modern HARC validation |
