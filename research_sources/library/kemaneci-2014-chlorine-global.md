...
| K14 | Under its stated `Te >> Th` assumption, Equation 18 writes elastic electron loss as `ne ni 3 Te (me/mi) k_el`. | This is retained as a named source-reproduction approximation. For an energy-dependent momentum-transfer cross section and stationary heavy target, direct two-body kinematics instead requires the same-table moment `2 me mi/(me+mi)^2 <sigma_m v E>`. The two forms must not be silently interchanged. |
| K15 | Table-4 reaction 18 excites `Cl(1P5/2)` and reaction 22 assigns a `1e5 s^-1` radiative decay; the source reproduction identifies the corresponding atomic-Cl emission near 139 nm. | Supplies a published-compilation emissivity proxy. Missing resonance trapping, collisional quenching, wall loss, and the unresolved physical state/cross section keep photon flux prediction false. |
