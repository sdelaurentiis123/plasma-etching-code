...
| WP5 calibration | controller pass; real proposal correctly blocked | Generic low/high discrepancy fitting and actual/predicted trust-ratio control pass. The checksum-bound readiness receipt refuses a new pair: remap selection, a current-epoch fine anchor, endpoint discrepancy, and current response directions are absent. The fixed R1.9 pair is the anchor location, not a new fit. |
