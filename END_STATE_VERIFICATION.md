...
| 3-D evolving profile | Mass/interface conservation, time-step/grid/ray convergence, topology robustness, charging feedback, and extracted profile metrics | A new one-step physical path carries dimensional boundary events through conserved SiO2 surface removal into material-gated level-set motion; a moving-plane manufactured gate reproduces flux x yield / formula density displacement to 0.002 um and reports CFL work. It deliberately refuses multi-step surface history until conservative state remapping is verified. Production charging feedback, remapping, topology and full grid/time/ray convergence remain open | **partial** |
