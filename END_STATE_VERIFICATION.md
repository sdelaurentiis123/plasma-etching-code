...
| Surface chemistry/state | SiO2 fluorocarbon mechanism with conserved coverages/film/mixed-layer state and sourced energy-angle yields; uniform-flux analytic limits; independent chemistry replay through unchanged core | A dimensional reduced SiO2/fluorocarbon kernel conserves complex sites and finite polymer inventory, implements the sourced threshold energy-angle law, carries parameter provenance/uncertainty, and refuses unsupported incident species. Its validity result now distinguishes executable manufactured/analytic parameter evidence from evidence that supports prediction in a declared physical domain; the current SiO2 test parameter set is explicitly nonpredictive. A checksum-bound Karahashi Figure-4 table now reproduces the measured F+/CF+/CF2+/CF3+ normal-incidence ladder through an opt-in closure and refuses unmeasured energy/angle/species support. The default aggregate-ion path is explicitly declared species-agnostic and nonvalidating. Takada Figure 3 separately records stable-C5F8/Ar+ co-incidence as an analog mechanism constraint, not a C4F6 law. Every reduced SiO2 and tabulated Si step now returns an exact material-origin exchange ledger: removed material equals routed outgoing plus chemically unresolved inventory face-by-face, and unresolved products are explicitly ineligible for redeposition. This closes bookkeeping without inventing SiF4/CO/CO2/COF2 branching. A general versioned surface-interaction table supports units-explicit axes, uncertainty, exact serialization replay, leave-one-out interpolation audits, machine-precision endpoint handling, and default extrapolation refusal. The CC-BY OSTI 2589032 archive is checksum-pinned; its byte-exact Si-Cl2-Ar+ sputter, RIE, and ALE product tables replay with MD uncertainty and fixed-condition provenance. A source-text plus PIL/Poppler audit corrected the ALE product condition from 80 eV to 215 eV: Figures 13--14 and all ten independently recovered Si/SiCl marker coordinates agree with `Products.csv` within 1.25 rendered pixels, while Figure 12 is the separate 80 eV morphology sequence. A dose-window operator now integrates the released Si, SiCl, SiCl2, and Cl yields without interpolating between window centres; compound-product stoichiometry closes both Si and Cl ledgers exactly, refuses an insufficient preloaded-Cl inventory, and refuses continuation beyond the released dose. Its 0.416 nm equivalent removal is a direct integral of the source MD products, explicitly not a held-out depth prediction or experiment. A sourced Si-Cl2-Ar+ mechanism runs two evolving 3-D steps through the unchanged transport, species filtering, generic conservative state remap, and level-set engine; at the released 100 eV, normal-incidence, 10:1 flux-ratio node its mean velocity matches the archived yield within 1% and reports prediction-supporting parameter evidence. It refuses unreleased energy, angle, ratio, and species. This demonstrates chemistry extensibility in one narrow second-chemistry domain, not broad Si prediction; stable C4F6 co-incidence, large-ion impact fragmentation, SiO2 crosslinking, resolved complex stoichiometry, F sequencing, product launch distributions/transport, and mask chemistry remain explicit omissions | **partial** |
