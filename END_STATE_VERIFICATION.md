...
| 3-D evolving profile | Mass/interface conservation, time-step/grid/ray convergence, topology robustness, charging feedback, and extracted profile metrics | The dimensional path carries boundary events through conserved SiO2 surface removal into material-gated level-set motion. A material-local remap now preserves integrated complex/polymer/removal state to machine precision, keeps coverage bounded, fingerprints every mesh handoff, and refuses topology/material/displacement violations. Moving-plane gates reproduce 0.020 um in one step within 0.002 um and 0.040 um in two steps within 0.004 um. Production charging feedback, topology-event physics, and full grid/time/ray convergence remain open | **partial** |
