...
| Charged-particle/neutral transport | Conservation, no tunnelling, energy error, forward/adjoint reciprocity, collision/re-emission balance, and sample/grid refinement with uncertainty | Strong collisionless mover/reciprocity gates; reflection/re-emission and collisional charged transport are incomplete | **partial** |
| Electrostatics and charging | Compatible deposition/field bases, variable permittivity, conductor equipotential, dielectric storage/conductivity, charge conservation, converged current balance, initialization/grid/sample invariance | Compatible nodal endpoint deposition converges and restart-certifies one nontrivial filled 2-D voltage-root trench. An opt-in physical Q1 mode now stores SI surface charge, solves variable-permittivity Poisson with reusable factorization, passes analytic capacitance/residual gates, and exactly replays a full estimator checkpoint; Poisson-mode AR/grid/sample convergence, conductivity, and SEE remain open | **partial** |
| Surface chemistry/state | SiO2 fluorocarbon mechanism with conserved coverages/film/mixed-layer state and sourced energy-angle yields; uniform-flux analytic limits; independent chemistry replay through unchanged core | Several reduced chemistry modules exist; the 3-D public default is SF6/O2 silicon physics, not a validated unified SiO2 fluorocarbon state model | **open** |
...
| Performance | Reproducible warmed CPU/GPU benchmarks; accuracy-matched competitor comparison; scaling with cells/faces/AR/species; device-residency and memory profile; deterministic replay | Historical ~1 s 3-D and ViennaPS comparisons exist; current charging path is CPU-only in this environment and end-to-end accuracy-matched benchmark is not current | **open** |
