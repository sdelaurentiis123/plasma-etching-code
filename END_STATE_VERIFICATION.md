...
| Surface chemistry/state | SiO2 fluorocarbon mechanism with conserved coverages/film/mixed-layer state and sourced energy-angle yields; uniform-flux analytic limits; independent chemistry replay through unchanged core | A dimensional reduced SiO2/fluorocarbon kernel conserves complex sites and finite polymer inventory, implements the sourced threshold energy-angle law, carries parameter provenance/uncertainty, and refuses unsupported incident species. A general versioned surface-interaction table supports units-explicit energy/angle/flux-ratio/state axes, uncertainty, constrained product branching, exact serialization replay, and default extrapolation refusal. The CC-BY OSTI 2589032 archive is now checksum-pinned; its byte-exact Si-Cl2-Ar+ sputter, RIE, and ALE product tables load with MD uncertainty and fixed-condition provenance, replay every node, and refuse out-of-domain energy/flux ratio. This closes manufactured-only table validation, not chemistry generality: the Si table is not yet coupled through the unchanged evolving-feature state interface, while omitted SiO2 crosslinking, resolved complex stoichiometry, F sequencing, redeposition, and mask chemistry remain explicit | **partial** |
